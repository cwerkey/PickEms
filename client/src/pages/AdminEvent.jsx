...
export default function AdminEvent() {
  ...
  const [editingNominee, setEditingNominee] = useState(null); // id -> {name, subtitle}
  ...
  const handleUpdateNominee = async (nomId) => {
    const { name, subtitle } = editingNominee;
    if (!name?.trim()) return;
    try {
      await api.updateNominee(nomId, { name, subtitle });
  ...
  const handleJsonEditorSave = async (replace) => {
    try {
      ...
      if (replace) {
        ...
        toast.success('Categories replaced');
      } else {
  ...
  return (
    <div className="page">
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: '1.1rem', marginBottom: 14 }}>PARTICIPANTS</h3>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: participants.length && nonParticipants.length ? 12 : 0 }}>
          {participants.map(p => (
            <div key={p.id} style={{
        ...
        {nonParticipants.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted)', width: '100%', marginBottom: 4 }}>Add:</span>
      ...
      {/* ── Correct Answers ── */}
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: '1.1rem', marginBottom: 6 }}>CORRECT ANSWERS</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 14 }}>
          Click the winning nominee to mark it correct. Click again to clear.
        </p>
        {event.categories?.map(cat => (
          <div key={cat.id} style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 6, fontWeight: 600 }}>{cat.name}</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
      ...
      {/* ── Category / Nominee Editor ── */}
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: '1.1rem', marginBottom: 14 }}>EDIT CATEGORIES & NOMINEES</h3>
        ...
        {event.categories?.map(cat => (
          <div key={cat.id} style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingLeft: 8 }}>
              {cat.nominees.map(nom => (
                <div key={nom.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {editingNominee?.id === nom.id ? (
                    ...
                    <>
                      <span style={{ flex: 1, fontSize: 13, color: 'var(--text-dim)' }}>
                        {nom.name}{nom.subtitle && <span style={{ color: 'var(--text-muted)' }}> — {nom.subtitle}</span>}
                      </span>
                      <button className="btn btn-ghost btn-sm" onClick={() => setEditingNominee({ id: nom.id, name: nom.name, subtitle: nom.subtitle || '' })}>Edit</button>
                      <button className="btn btn-danger btn-sm" onClick={() => handleDeleteNominee(nom.id)}>✕</button>
                    </>
                  )}
                </div>
              ))}
        ...
        >
          <div className="card fade-in" style={{ width: '100%', maxWidth: 720, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
              Edit the JSON below and save. Use "Replace All" to overwrite existing categories, or "Add" to append.
            </p>
            <textarea
              value={jsonText}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary" onClick={() => handleJsonEditorSave(false)}>
                Add & Save
              </button>
            </div>
          </div>
