...
const db = require('../db');
const { requireAdmin, requireAuth, permit } = require('../middleware');
const { hashPassword } = require('../auth');

// ── Helper ────────────────────────────────────────────────────
// Check if a user is admin OR is MoC for a specific event
function isAdminOrEventMoC(userObj, eventId) {
  if (userObj.role === 'admin') return true;
  const ep = db.prepare(
    "SELECT participant_role FROM event_participants WHERE event_id = ? AND user_id = ?"
  ).get(eventId, userObj.id);
  return ep?.participant_role === 'moc';
}
...
router.post('/users', requireAdmin, permit('username', 'password', 'display_name', 'role'),
  async (req, res) => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
      return res.status(400).json({ error: 'Username must be alphanumeric or underscores' });
    }
    if (password.length < 6 || password.length > 128) {
      return res.status(400).json({ error: 'Password must be 6–128 characters' });
    }
    // Only admin and user are valid account-level roles now
    // MoC is per-event, not per-account
...
router.put('/join-requests/:id', requireAdmin, permit('status'), (req, res) => {
  ...
  db.transaction(() => {
    db.prepare("UPDATE join_requests SET status = ?, resolved_at = datetime('now') WHERE id = ?")
      .run(status, req.params.id);
    if (status === 'approved') {
      db.prepare(
        'INSERT OR IGNORE INTO event_participants (event_id, user_id) VALUES (?, ?)'
      ).run(request.event_id, request.user_id);
    }
  })();
  res.json({ success: true });
});
...
router.get('/events/:id/participants', requireAuth, (req, res) => {
  // Admin sees all; event MoC also needs this for the event page
  if (!isAdminOrEventMoC(req.user, req.params.id)) {
    return res.status(403).json({ error: 'Insufficient permissions' });
  }
  const participants = db.prepare(`
    SELECT u.id, u.display_name, u.username, ep.joined_at, ep.participant_role
...
router.get('/participants/matrix', requireAdmin, (req, res) => {
  const users = db.prepare(
    'SELECT id, display_name, username FROM users ORDER BY display_name'
  ).all();
  const events = db.prepare(
    'SELECT id, name, is_locked, is_archived FROM events ORDER BY created_at DESC'
  ).all();
  const participants = db.prepare(
    'SELECT event_id, user_id, participant_role FROM event_participants'
  ).all();
  // matrix[userId][eventId] = participant_role string
  const matrix = {};
  for (const p of participants) {
    if (!matrix[p.user_id]) matrix[p.user_id] = {};
    matrix[p.user_id][p.event_id] = p.participant_role;
  }
...
router.post('/events/:id/participants', requireAdmin, permit('user_id'), (req, res) => {
  ...
  db.prepare(`UPDATE join_requests SET status = 'approved', resolved_at = datetime('now')
...
// Set a participant's role within a specific event (participant or moc)
router.put('/events/:eventId/participants/:userId/role',
  requireAdmin,
  permit('participant_role'),
  (req, res) => {
    const { participant_role } = req.body;
    if (!['participant', 'moc'].includes(participant_role)) {
      return res.status(400).json({ error: 'participant_role must be participant or moc' });
    }
    // User must already be a participant before being made MoC
    const ep = db.prepare(
      'SELECT id FROM event_participants WHERE event_id = ? AND user_id = ?'
    ).get(req.params.eventId, req.params.userId);
    if (!ep) {
      return res.status(404).json({ error: 'User is not a participant in this event' });
    }
    db.prepare(
      'UPDATE event_participants SET participant_role = ? WHERE event_id = ? AND user_id = ?'
    ).run(participant_role, req.params.eventId, req.params.userId);
    res.json({ success: true });
  }
);
...
router.post('/events', requireAdmin, permit('name', 'description', 'event_date', 'lock_time'),
  (req, res) => {
    const { name, description, event_date, lock_time } = req.body;
    if (!name?.trim()) return res.status(400).json({ error: 'Event name required' });
    const result = db.prepare(
...
// Update event — admin can do everything; event MoC can only lock (not unlock)
router.put('/events/:id', requireAuth, permit('name', 'description', 'event_date', 'lock_time', 'is_locked', 'is_archived'),
  (req, res) => {
    ...
    if (req.user.role !== 'admin') {
      // Must be MoC for this specific event
      if (!isAdminOrEventMoC(req.user, req.params.id)) {
        return res.status(403).json({ error: 'Insufficient permissions' });
      }
      // MoC can only set is_locked = 1 (lock), never unlock or change other fields
      const { is_locked } = req.body;
      if (is_locked === undefined || is_locked === 0 || is_locked === '0') {
        return res.status(403).json({ error: 'MoC can only lock events, not unlock or edit them' });
      }
      db.prepare("UPDATE events SET is_locked = 1, updated_at = datetime('now') WHERE id = ?")
        .run(req.params.id);
    ...
    db.prepare(`UPDATE events SET
      ...
      updated_at = datetime('now')
      WHERE id = ?`
    ).run(name || null, description || null, event_date || null, lock_time || null,
      is_locked ?? null, is_archived ?? null, req.params.id);
...
router.post('/events/:id/categories', requireAdmin, permit('name'), (req, res) => {
  ...
  const maxOrder = db.prepare(
    'SELECT COALESCE(MAX(sort_order),0) as m FROM categories WHERE event_id = ?'
  ).get(req.params.id).m;
  const result = db.prepare(
...
router.post('/categories/:id/nominees', requireAdmin, permit('name', 'subtitle'), (req, res) => {
  ...
  const maxOrder = db.prepare(
    'SELECT COALESCE(MAX(sort_order),0) as m FROM nominees WHERE category_id = ?'
  ).get(req.params.id).m;
  const result = db.prepare(
    'INSERT INTO nominees (category_id, name, subtitle, sort_order) VALUES (?, ?, ?, ?)'
  ).run(req.params.id, name.trim(), subtitle?.trim() || null, maxOrder + 1);
...
router.put('/nominees/:id', requireAdmin, permit('name', 'subtitle', 'sort_order'), (req, res) => {
  ...
  if (!name?.trim()) return res.status(400).json({ error: 'Name required' });
  db.prepare(
    'UPDATE nominees SET name = ?, subtitle = ?, sort_order = COALESCE(?, sort_order) WHERE id = ?'
...
// Set correct answer — admin OR event MoC
router.put('/categories/:id/answer', requireAuth, permit('nominee_id'), (req, res) => {
  const cat = db.prepare('SELECT * FROM categories WHERE id = ?').get(req.params.id);
  if (!cat) return res.status(404).json({ error: 'Category not found' });
  if (!isAdminOrEventMoC(req.user, cat.event_id)) {
    return res.status(403).json({ error: 'Insufficient permissions' });
  }
